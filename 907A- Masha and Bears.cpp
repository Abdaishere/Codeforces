...
int const N = 1e5 + 10, mod = 1e9 + 7, oo = 1e9 + 10;
//-------------------------------------------------------------------------------------
...
int main() {
    ...
    cin >> f >> m >> s >> M;
    int n = M - 1, tmpf = f, tmpm = m, tmps = s;
    bool F = 0;
    for (int i = 1; i <= 200; i++) {
        if ((tmps * 2 >= i && tmps <= i) && (2 * M >= i && M <= i)) {
            s = i;
            for (int j = i + 1; j <= 200; j++) {
                if (tmpm * 2 >= j && tmpm <= j && 2 * M < j) {
                    m = j;
                    for (int q = j + 1; q <= 200; q++) {
                        if (tmpf * 2 >= q && tmpf <= q && 2 * M < q) {
                            f = q;
                            F = 1;
                            break;
                        }
